---
title: "Exploratory Data Analysis with Databricks"
format: html
---

This example shows easily accessing data in Databricks with Workbench Managed Credentials and dynamically generating SQL queries from R code.

Posit has created the [tidyverse](https://www.tidyverse.org/) which is a collection of some of the most widely used R packages for data science. The [dbplyr](https://dbplyr.tidyverse.org/) package is especially helpful for working with Databricks as it can take standard [dplyr](https://dplyr.tidyverse.org/) R code and dynamically convert to SQL pushed to Databricks
...
con <- dbConnect(
  odbc::databricks(),
  HTTPPath = "/sql/1.0/warehouses/ef4f89054a7fdcf3",
)
con
```
...
sc <- spark_connect(
  cluster_id = "0495-195338-1w0t2cgm",
  method = "databricks_connect"
)
sc
```
Query the data in a similar fashion as the SQL Warehouse or use spark specific functions

```{r}
filtered_data <- tbl(sc, in_catalog("demo", "default", "lending_club")) |> filter(zip_code == '996xx') |> collect()
filtered_data
```
